...
const CONFIG_FILENAMES = ["conductor.config.yaml", "conductor.config.yml", "conductor.yaml"];
...
export function loadConfigFromEnv(): ConductorConfig {
	...
	const baseConfig: ConductorConfig = {
		...
		routing: {
			strategy:
				(process.env.CONDUCTOR_ROUTING_STRATEGY as "auto" | "round-robin" | "explicit") || "auto",
		},
	};
	...
	if (mode === "single") {
		...
		if (!agentUrl) {
			throw new Error("CONDUCTOR_AGENT_URL is required when CONDUCTOR_MODE=single");
		}
		baseConfig.agent = {
		...
		if (!posseName) {
			throw new Error("CONDUCTOR_POSSE_NAME is required when CONDUCTOR_MODE=posse");
		}
		baseConfig.posse = {
			name: posseName,
			meilisearch: {
			...
			discovery: {
				...
				pollInterval: Number.parseInt(process.env.CONDUCTOR_DISCOVERY_INTERVAL || "30000", 10),
			},
		};
	}
