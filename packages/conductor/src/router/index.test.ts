...
import type { AgentResponse, Task } from "./index.ts";
...
function createMockRegistry(): MockAgentRegistry {
	...
	const registry = {
		...
		getHealthyAgents: () => {
			...
			return all.filter((a: EnrichedAgentRecord) => a.health.isResponsive);
		},
		getStats: () => ({
			total: agents.size,
			online: Array.from(agents.values()).filter(
				(a: EnrichedAgentRecord) => a.health.status === "healthy",
			).length,
			offline: 0,
			busy: Array.from(agents.values()).filter(
				(a: EnrichedAgentRecord) => a.health.status === "busy",
			).length,
			error: 0,
		}),
...
function createPosseModeConfig(
	strategy: "auto" | "round-robin" | "explicit" = "auto",
): ConductorConfig {
	return {
		mode: "posse",
		model: "moonshotai/kimi-k2.5",
...
describe("TaskRouter", () => {
	describe("single mode", () => {
		...
		beforeEach(() => {
			...
			router = createTaskRouter(
				config,
				undefined,
				mockClient as unknown as ReturnType<typeof createHttpAgentClient>,
			);
		});
	...
	describe("multi-agent routing", () => {
		...
		it("should route to multiple agents in parallel", async () => {
			...
			const results = await router.routeToMultiple(createMockTask({ id: "task-1" }), [
				"agent-1",
				"agent-2",
			]);
