...
const OPENCODE_MODELS: ModelOption[] = [
	...
	{ value: "openai/gpt-5.4", label: "GPT-5.4", hint: "OpenAI — latest flagship" },
...
function generateEnvTemplate(opts?: {
	...
	const lines = [
		...
		"# OpenRouter provides multi-model access (recommended)",
		"OPENROUTER_API_KEY=",
		"",
...
async function detectEnvironment(): Promise<EnvDetection> {
	...
	detectionLines.push("");
	detectionLines.push(
		`Meilisearch:  ${hasMeili ? "✅ running on :7701" : "⬚  not detected (will auto-install on first `randal serve`)"}`,
	);
