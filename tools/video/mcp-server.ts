...
server.tool(
  ...
  "Generate a video clip from text or reference image using Veo (or another configured provider)",
  {
    ...
    filename: z
      .string()
      .optional()
      .describe("Output filename (default: auto-generated UUID)"),
    provider: z
      .string()
      .optional()
      .describe("Video provider name (e.g. 'veo'). Uses first configured provider if omitted"),
  },
  async ({ prompt, duration, aspect_ratio, reference_image_path, output_dir, filename, provider }) => {
    try {
...
server.tool(
  ...
  "Concatenate video clips into a single video using ffmpeg",
  {
    ...
    transition_duration: z
      .number()
      .optional()
      .describe("Crossfade duration in seconds (default: 1). Only used when transition is 'crossfade'"),
  },
...
server.tool(
  ...
  "Compose a rich video with overlays, transitions, and text using Remotion",
  {
    ...
    template: z
      .string()
      .optional()
      .describe("Remotion composition ID (default: 'ScriptedVideo')"),
    project_dir: z
      .string()
      .optional()
      .describe("Path to an existing Remotion project. If omitted, copies remotion-template to a temp dir"),
  },
