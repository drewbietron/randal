...
import { generateImage, ImageGenerationError } from "../image-gen";
...
describe("end-to-end simple pipeline", () => {
  ...
  test("stage 1: generate a reference image via mocked fetch", async () => {
    ...
    expect((capturedBody as Record<string, unknown>).model).toBe("google/gemini-3.1-flash-image-preview");
  });
  ...
  test("full pipeline: image → clip → clip → stitch validation", async () => {
    ...
    const clip2 = await generateVideoClip(
      "Detective picks up phone, glances at it, puts it down",
      {
        provider: "mock",
        duration: 6,
        referenceImage: image.buffer,
...
describe("generate_clip tool reference image handling", () => {
  test("referenceImage Buffer is passed through to the provider", async () => {
    ...
    const result = await generateVideoClip(
      "Scene with reference image from file",
      {
        provider: "mock",
