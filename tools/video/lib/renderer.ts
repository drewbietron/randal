...
async function validateProjectDir(projectDir: string): Promise<void> {
	...
	if (!existsSync(resolvedDir)) {
		throw new RenderError(`Project directory does not exist: "${resolvedDir}"`, "MISSING_PROJECT");
	}
...
function classifyRenderError(
	...
	if (lowerStderr.includes("composition") && lowerStderr.includes("not found")) {
		return {
...
export async function renderVideo(
	...
	if (!projectDir || projectDir.trim() === "") {
		throw new RenderError("projectDir must be a non-empty string.", "INVALID_ARGUMENTS");
	}

	if (!compositionId || compositionId.trim() === "") {
		throw new RenderError("compositionId must be a non-empty string.", "INVALID_ARGUMENTS");
	}

	if (!outputPath || outputPath.trim() === "") {
		throw new RenderError("outputPath must be a non-empty string.", "INVALID_ARGUMENTS");
	}

	if (inputProps === null || typeof inputProps !== "object" || Array.isArray(inputProps)) {
		throw new RenderError("inputProps must be a non-null object.", "INVALID_ARGUMENTS");
	}
