...
import { VIDEO_DEFAULTS, validateVideoScript } from "../lib/types";
...
export function ScriptedVideo({ script }: ScriptedVideoProps): React.ReactElement {
  ...
  for (let i = 0; i < script.scenes.length; i++) {
    ...
    const TransitionWrapper =
      scene.transition && i > 0
        ? getTransitionComponent(scene.transition.type)
        : null;
    ...
    const sceneContent = React.createElement(
      ...
      scene.overlay
        ? React.createElement(OverlayText, { overlay: scene.overlay })
        : null,
    );
