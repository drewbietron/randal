  ...
  for (let i = 0; i < script.scenes.length; i++) {
    ...
    switch (scene.type) {
      ...
      case "color":
        ...
        break;
      default:
        errors.push(`${prefix}: unknown type "${scene.type}". Expected "image", "video", "text", or "color".`);
    }

    if (scene.transition) {
      const validTypes: TransitionType[] = ["crossfade", "slide-left", "slide-right", "zoom-in", "cut"];
      ...
      if (scene.transition.duration <= 0 || !Number.isFinite(scene.transition.duration)) {
        errors.push(`${prefix}: transition duration must be positive (got ${scene.transition.duration}).`);
      }
      if (scene.transition.duration >= scene.duration) {
        errors.push(`${prefix}: transition duration (${scene.transition.duration}s) must be less than scene duration (${scene.duration}s).`);
      }
    }
  }
