...
function calculate(contextLimit: number, outputLimit: number, modelName: string) {
  ...
  const estCostPerStep = (estInputPerStep * pricing.input + estOutputPerStep * pricing.output) / 1_000_000
  // Planning turns use more input (reading), less output
  const estInputPerPlanTurn = 20000
  const estOutputPerPlanTurn = 10000
  const estCostPerPlanTurn = (estInputPerPlanTurn * pricing.input + estOutputPerPlanTurn * pricing.output) / 1_000_000
